...
const ManageProductsScreen = () => {
  ...
  const handleBoost = async (type, id) => {
    try {
      ...
      const response = await fetch(`https://main-matrixai-server-lujmidrakh.cn-hangzhou.fcapp.run/${endpoint}`, {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('https://main-matrixai-server-lujmidrakh.cn-hangzhou.fcapp.run/getOwnerAllProducts', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const handleRemoveProduct = async (type, id) => {
    setRemoving(id);
    try {
      await fetch('https://main-matrixai-server-lujmidrakh.cn-hangzhou.fcapp.run/removeProduct', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
