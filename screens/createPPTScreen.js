    ...
    <SafeAreaView style={[styles.container, {backgroundColor: colors.background}]}>
      {loading && (
        <View style={styles.uploadingOverlay}>
          ...
          <Text style={[styles.uploadingText, {color: colors.text}]}>{t('creating')}...</Text>
        </View>
      )}
      <Text style={[styles.heading, {color: colors.text}]}>
        {loading ? t('generatingPPT') : t('pptGenerated')}
      </Text>
      ...
      {!loading && (
        <View style={styles.buttonsContainer}>
          ...
          >
            <Text style={styles.generateText}>{t('ok')}</Text>
          </TouchableOpacity>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  subtext2: {
    fontSize: 16,
    color: "#E66902FF", // Consider replacing with a theme color
    textAlign: "center",
  },
  imageContainer: {
    width: "70%",
