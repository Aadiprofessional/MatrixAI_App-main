...
const HelpScreen = ({ route, navigation }) => {
  ...
  const commonIssues = [
    t('paymentNotReflected'),
    t('coinsNotCredited'),
    t('planActivationFailed'),
    t('needRefundAssistance'),
    t('otherIssues'),
  ];
  ...
  const handleSubmit = async () => {
    ...
    if (!uid) {
      Alert.alert('Error', t('userIdNotAvailable'));
      return;
    }
    
    try {
      const response = await fetch('https://main-matrixai-server-lujmidrakh.cn-hangzhou.fcapp.run/api/user/getHelp', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        Alert.alert('Error', data.message || `${t('failedToSubmitRequest')} (Status: ${response.status})`);
      }
    } catch (error) {
      console.log('Network Error:', {
        message: error.message || 'Unknown error'
      });
      Alert.alert('Error', `${t('errorOccurredDetail')} ${error.message || 'Unknown error'}`);
  ...
  return (
    <SafeAreaView style={[styles.container, {backgroundColor: colors.background}] }>
      ...
      <TouchableOpacity style={styles.backButton} onPress={() => navigation.goBack()}>
        ...
        <Text style={[styles.headerTitle, {color: colors.text}]   }>{t('getHelp')}</Text>
      </View>
      ...
      >
        ...
        >
          <View style={[styles.orderInfoCard, {backgroundColor: colors.background2, borderColor: colors.border}]}>
            ...
            <Text style={[styles.orderInfoText, {color: colors.text}]}>{t('orderNumber')}{orderId}</Text>
          </View>

          <Text style={[styles.sectionTitle, {color: colors.text}]}>{t('whatIssueAreFacing')}</Text>
          ...
          <TouchableOpacity style={[styles.submitButton, {backgroundColor: colors.primary}]} onPress={handleSubmit}>
            <Text style={[styles.submitButtonText, {color: colors.text}]}>{t('submitRequest')}</Text>
          </TouchableOpacity>
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
