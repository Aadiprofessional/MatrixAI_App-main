...
const OTPCodeScreen2 = ({ route, navigation }) => {
    ...
    const handleVerify = async () => {
        ...
        if (enteredOtp.length === 6 && phone) {
            try {
                ...
                const response = await fetch('https://main-matrixai-server-lujmidrakh.cn-hangzhou.fcapp.run/verifyPhoneOtp', {
                    method: 'POST',
                    headers: {
    ...
    const handleLogin = async () => {
        ...
        setLoading(true);
        try {
            const response = await fetch('https://main-matrixai-server-lujmidrakh.cn-hangzhou.fcapp.run/sendPhoneOtp', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
